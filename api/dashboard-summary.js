...
export default async function handler(req, res) {
  ...
  try {
    ...
    // For 1W: prepend range-start point and drop any points before it so combined chart backfills at combined opening
    if (rangeVal === '1W' && combinedOpeningBalance > 0 && combinedHistory.length > 0) {
      ...
      const onOrAfterRangeStart = combinedHistory.filter(
        (p) => new Date(p.time).getTime() >= rangeStartMs
      );
      combinedHistory = [
